...
| 4 | **Relay error/load counters observable** — fine-grained per-error counters remain available via [`RelayHandle::debug_stats`] for in-process tests only; external surfaces should use [`MetricsExportGate`] (default 30s cadence / quantize / suppress ingress drops) over raw [`RelayHandle::coarse_stats`]. Residual GPA risk for privileged / high-res observers under flood. | `aegis-relay::metrics_export` · `RelayCoarseStats` | **Low–medium** (was Medium) |
...
| Finding | Location | Status | Sev |
|---------|----------|--------|-----|
| Inbound/outbound `mpsc` channels (capacity 64). | `node::RELAY_CHANNEL_CAPACITY`, `aegis-node` `main`, `net::run_inbound_connection` | **Mitigated (2026-07-17)** — bounded channels; on full, **drop-newest** via `try_send_drop_newest` (never block forever under flood). Coarse counters: `RelayCoarseStats::queue_dropped` (outbound), `QueueDropStats` (inbound). **Per-peer weighted fair inbound + outbound (2026-07-17):** each TCP connection has a `PER_PEER_INBOUND_CAPACITY` queue; each next-hop has `PER_PEER_OUTBOUND_CAPACITY`; weighted credit RR (WFQ-style) drains into the shared mix inbound and before egress TCP; weights from peer-health success rate (default `1`, unhealthy lower). Residual: discrete weight quanta not continuous GPS. | **Low** |
...
| ~~**Implementation uses only `primary_guard()` (g=1 effective)**~~ — held set is `GUARD_SET_SIZE=3`; path builders pin via `entry_guard_for_packet` (sticky primary default; `GuardPinMode::Rotate` for plateau-style cycling). Production: `build_bound_path_pruned_with_guards` + reputation-weighted g-set. | `guards::GuardSelector::{guard_set,primary_and_backups,entry_guard_for_packet}`, `path::select_path_indexed_for_tests` | **Mitigated (2026-07-17)** — g=3 set + production multi-guard helpers. Residual: sticky primary still g=1 *entry pin* (GPA learns one identity). Unfiltered `select_path_for_tests` / `GuardSelector::new_for_tests` gated behind `test-utils` (2026-07-17). | Low (was Medium) |
| Path inner hops fresh CSPRNG per packet. | `path::select_path_indexed_impl` | **Mitigated**. | — |
...
| Sybil flood raises unfiltered sticky-primary capture ≈ layer-1 Sybil fraction; held g=3 set tracks `1-(1-c)^g`. **Admission + reputation** (rate limit 5/24h, probation 0.1, M-of-N, `new_reputation_weighted` / `build_bound_path_pruned_with_guards`) block **fresh** Sybils so g=3+rep set capture ≈0 under flood. Unfiltered APIs (`select_path_for_tests`, `GuardSelector::new_for_tests`) compile only under `cfg(test)` or `test-utils` (default off) — production cannot call them; residual science in `sybil_majority_flood_unfiltered_g1_saturates`. | `guards::guard_exposure_plateau`, `GUARD_SET_SIZE`, `tests/sybil_admission.rs` | **Mitigated (2026-07-17)** — production multi-guard + rep filter + API fencing closes paper gap; residual is honest-pool failure at extreme c when layer-1 is majority-Sybil (theoretical, not reachable via production APIs). | **Low** (was High / Medium) |
...
| ZK proofs do not hide relay identity (module docs). | `zk.rs` / `AnonymousReputationPresentation` / `NullifierRegistry` | **Partial (2026-07-17)** — `present_anonymous` / `verify_anonymous` + local file-backed `NullifierRegistry` / `verify_anonymous_and_spend` (per-epoch replay prevention); identity binding via out-of-band nullifier + `score_commitment`. Residual: no full AC / consensus issuer / cross-node nullifiers / wire gossip — see `docs/ops/anonymous_reputation.md`. | Low–medium |
...
| Finding | Location | Status | Sev |
|---------|----------|--------|-----|
| Emitter queue unbounded on `enqueue`. | `emitter::ConstantRateEmitter`, `EmitterConfig::max_backlog`, `PacedSession` bounded channel | **Mitigated (2026-07-17)** — default cap 256 real cells (`DEFAULT_MAX_BACKLOG`, ~14 Sphinx packets); `enqueue` / `enqueue_cell` return [`BacklogFullError`](../../crates/aegis-client/src/emitter.rs) at cap (Mode-1: fail send over unbounded memory); [`PacedSession`](../../crates/aegis-client/src/session.rs) uses bounded `mpsc` + `try_send`. Residual: lab `enqueue_drop_newest` drops with counter; driver defense-in-depth may drop if channel/emitter disagree. | — |
...
**Fix (2026-07-12):** `ReputationScore::PROBATIONARY` (0.1) seeded at signed admission; `RosterAdmissionPolicy` default **5 admissions / 24h** (`AdmissionRateLimitExceeded`). Reputation-filtered path/guard selection now excludes fresh Sybils. **Fix (2026-07-17):** M-of-N threshold admission (`ThresholdConsortium`, default production path `admit_threshold_signed`); signed roster records include SHA3-256 hybrid KEM public-key commitments verified via `RelayRecord::binds_kem_public`; live relay peer-health drains update the shared EWMA ledger via `feed_peer_outcomes` / `record_aggregate` (optional JSON persistence via `[reputation] ledger_path`). **Fix (2026-07-17, workstream #9):** `GUARD_SET_SIZE=3`; `GuardSelector::{guard_set, primary_and_backups, entry_guard_for_packet}`; sticky primary (default) or rotate pin; production `build_bound_path_pruned_with_guards` builds reputation-weighted multi-guard + pruned path. **Fix (2026-07-17, audit A-item):** unfiltered `select_path` / `GuardSelector::new` renamed to `*_for_tests`, deprecated, and gated behind `cfg(test)` or `test-utils` (mirrors unsigned `admit_for_tests` fencing); production builds cannot link them without explicitly enabling the feature.

**Conclusion:** Held **g=3** set exposure tracks `guard_exposure_plateau(c, 3)`. Production defaults combine multi-guard + reputation filtering so fresh Sybils do not enter the g-set or pruned paths. Sticky primary remains a g=1 *entry pin* (GPA learns one identity per epoch) by design; rotate mode is available for plateau-style cycling. Unfiltered APIs remain available only for Sybil science tests (`#[allow(deprecated)]`, `test-utils` feature).
...
```
ConsortiumKey(s) ──M-of-N sign──► RelayRoster (+ KEM commitment) ──filters──► Topology ──feeds──► build_bound_path_pruned_with_guards / rep-weighted GuardSelector
                              ▲                           │
...
7. ~~Wire `AnomalyDetector` to admission/pruning decisions.~~ **Partial (2026-07-17):** `RelayPruningPolicy` demotes on anomaly; path/guard `*_pruned` selection + [`build_bound_path_pruned`](../../crates/aegis-topology/src/path.rs) use `is_eligible`; admission gating **Done** via [`RelayRoster::admit_signed_pruned`](../../crates/aegis-topology/src/roster.rs) / [`admit_threshold_signed_pruned`](../../crates/aegis-topology/src/roster.rs). [`PeerHealthTracker`](../../crates/aegis-relay/src/peer_health.rs) feeds metrics (`aegis-node` every 30s). **Partial (2026-07-17):** signed [`PeerHealthAdvert`](../../crates/aegis-relay/src/health_gossip.rs) over hop links with lightweight `majority_k` median merge (default K=2, half-weight) — see [`docs/ops/health_gossip.md`](ops/health_gossip.md). Residual: not BFT; callers must use pruned APIs; unidentified inbound not attributed.
8. ~~Relay-side timestamp instrumentation for shapeability at **post-shaping** vantage (Phase 8 notes §5).~~ **Done (2026-07-17):** optional `trace.path` in `aegis-node` TOML → [`RelayForwardTrace`](../../crates/aegis-relay/src/trace.rs). **Done (2026-07-18):** paced multi-process re-capture committed at `sim/data/real_multiprocess_relay_forward_trace.csv` + `sim/data/real_multiprocess_relay_forward_trace.analysis.json` (1 s `shapeability_report`: relay tier **unshapeable** CV≈3.06 vs client-send **feasible** CV≈1.39–1.48 — loopback vantage/bin-width artifact; see Phase 8 notes §5). Residual: mix relays keep trace off; non-loopback re-capture not committed.
...
constant-time review are **closed**. The follow-on **research/ops leftovers wave
is closed** except B-class **External** items (hardware TEE SDK, multi-org BFT,
full dudect lab, full AC issuer) — see
[`ops/RESEARCH_OPS_STATUS.md`](ops/RESEARCH_OPS_STATUS.md) and
[`AEGIS_research_ops_hardening_plan.md`](AEGIS_research_ops_hardening_plan.md).
In-tree residuals are Partial/Mitigated with honest leftover notes — not
unfinished product wiring:
...
| Consortium authority key ceremony / PKI | ops | **Partial** — runbook + `aegis-ceremony` + optional GF(256) Shamir seed shares (`docs/ops/consortium_key_ceremony.md`); residual: no HSM / MPC / proactive refresh |
...
| Cross-relay health gossip / reputation consensus | research | **Partial (2026-07-17)** — signed `PeerHealthAdvert` + `majority_k` median merge (`docs/ops/health_gossip.md`); residual: lightweight majority only, not BFT |
| ZK anonymous reputation (hide relay identity) | research | **Partial** — anonymous presentation + local `NullifierRegistry`; full AC / consensus issuer **External** (`docs/ops/anonymous_reputation.md`) |
